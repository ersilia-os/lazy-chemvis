...
class UMAPSurrogate(object):
    ...
    def fit(self):
        ...
        """
        console.print(Panel.fit("🚀  UMAP Surrogate — Training Pipeline", style="bold cyan"))

        # 1. DATA LOADING & ALIGNMENT
        logger.info("Loading ECFP features and UMAP projector...")
        ...
        if os.path.exists(metadata_path):
            ...
            valid_indices = metadata["original_index"].values
            X = X[valid_indices]   # fancy-index → new copy, original stays in ecfp_feat
            del metadata, valid_indices
